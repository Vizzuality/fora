...
const LegalSelected: React.FC<GeographicSelectedProps> = ({}: GeographicSelectedProps) => {
  ...
  console.log('geographicData', geographicData);

  const handleSelectGeo = useCallback((value) => {
    console.log({ value });
  }, []);

  const formatedSubgeo = useMemo(() => {
    console.log({ subgeographicsData });
    // subgeographicsData.map((item) => {
    //   console.log(item.name);
    //   return item.name;
    // });
    _.map(subgeographicsData, 'name');
  }, [subgeographicsData]);
  console.log('formatedSubgeo', formatedSubgeo);
  return (
    <div
      className={cx({
