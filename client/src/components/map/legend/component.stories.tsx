      ...
      {sortedItems.map((i) => {
        ...
        return (
          <LegendItem key={i.id} {...i}>
            {type === 'basic' && <LegendTypeBasic className="visible text-sm" items={items} />}
            {type === 'choropleth' && <LegendTypeChoropleth className="text-sm" items={items} />}
            {type === 'gradient' && <LegendTypeGradient className="text-sm" items={items} />}
          </LegendItem>
        );
      })}
    </Legend>
