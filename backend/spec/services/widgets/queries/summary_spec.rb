...
RSpec.describe Widgets::Queries::Summary do
  ...
  describe "#call" do
    ...
    it "contains correct data" do
      expect(subject.call[:data].first[:value]).to eq(1)
      expect(subject.call[:data].second[:value]).to eq(2)
      expect(subject.call[:data].third[:value]).to eq(20)
      expect(subject.call[:data].fourth[:value]).to eq(10)
    end
  end
end
